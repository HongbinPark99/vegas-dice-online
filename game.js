...
class VegasGame {
  /**
   * @param {Array<{id:string, name:string}>} players 2-6 players
   */
  constructor(players) {
  ...
  _finishGame() {
    ...
    let best = -Infinity;
    Object.values(this.players).forEach((p) => { if (p.money > best) best = p.money; });
    this.winnerIds = Object.values(this.players).filter((p) => p.money === best).map((p) => p.id);
